...
def exec_lbp(pValue,rValue):
    ...
    trainFeatures = extractLBPTextureFeatures(trainImages,pValue,rValue)
    ...
    testFeatures = extractLBPTextureFeatures(testImages,pValue,rValue)
...
def extractLBPTextureFeatures(images, pValue,rValue):
    bar = Bar('[INFO] Extrating LBP texture features...',max=len(images),suffix='%(index)d/%(max)d  Duration:%(elapsed)ds')
    featuresList = []
    for image in images:
        if (np.ndim(image) > 2): # > 2 = colorida
            image = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
        lbp = local_binary_pattern(image, pValue, rValue, method="uniform")
        (hist, _) = np.histogram(lbp.ravel(),bins=np.arange(0, pValue + 3),range=(0, pValue + 2))
